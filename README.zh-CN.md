...
默认策略是：**有关联就续写，只有真正跨越任务边界才新建**。同一项目的规划、实现、测试、调试、评审、
文档和追问通常都留在同一个精确 DSH 会话里。再次调用 MCP、上一轮已经结束或间隔了一段时间，都不是
重新开会话的理由。
...
- 只有用户明确要求新会话、目标明显无关、工作区或信任边界变化，或者需要隔离运行一个独立并行任务时，
  才新建会话。新建时指定已注册的 `workspace_id` 或绝对 `cwd`。如果工具参数和安装默认值都没有，
  `dsh_start_task` 返回 `task-target-required`，Codex 应先列出工作区/会话，再询问用户。
- 当前 Codex 任务没有绑定精确会话时，可以读取有界的会话元数据和历史；只有一个候选同时明确匹配工作区
...
`dsh_start_task` 永远表示“新对话”，`dsh_send` 永远表示“继续这个精确对话”。
...
- `workspace-write`：DSH Web 默认值；文件修改限制在会话工作区及受支持的临时目录。该文件策略不限制
  读取、网络访问和进程可见性。
- `danger-full-access`：不受 DSH 文件沙箱限制；对已注册工作区明确选择后即可使用。
